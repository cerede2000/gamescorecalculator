...
function isTrue(v: Maybe): boolean {
  if (v === null) return false
  if (v.type !== 'BOOLEAN') throw new Error(`condition non booléenne : ${v.type}`)
  // une valeur hors domaine est une ERREUR, jamais un « faux » silencieux
  if (v.value !== 'true' && v.value !== 'false')
    throw new Error(`BOOLEAN hors domaine : « ${v.value} » — attendu true ou false`)
  return v.value === 'true'
}
